...
Every problem hit while building this lab and what actually fixed it. Entries are written as they
happened, in order, including the wrong turns.

This is the file I would read first. `RUNBOOK.md` describes the build that works. This describes
the build that actually happened, and the entries exist so someone repeating this does not lose the
same time I did.

Each entry follows the same shape: what I was doing, what happened, what I tried, what actually
fixed it, and the lesson if there was one. Exact error strings are reproduced verbatim, because
those are what people search for.

---

## Session 1: 7/23/2026, prep and VM creation
...
### [PREP] Old VMs showing "Inaccessible" with VERR_PATH_NOT_FOUND
...
## Session 2: 7/24/2026 to 7/25/2026, domain build and client join
...
### [SUPPLEMENTAL] VirtualBox throws VBOX_E_INVALID_VM_STATE when restoring a snapshot with the safety checkbox enabled
...
**Downgrading to Server 2019 or Windows 10 is not a fix.** Those evals expire identically, and
...
## Session 5: 7/31/2026, account lockout and password management extension lab
